...
- ✅ **卧室端联网 / MQTT 已验证**：电脑 USB 连接 C3；C3 在卧室可扫描并连接
  `CU_hSUF1`，拿到 `192.168.1.11`，连接小主机 MQTT `192.168.1.26:1883`
  成功，已订阅 `jk-bms/set`。
- ⏳ **BLE 尚未在卧室验证**：BMS 在工具间，当前 `conn=0` 是正常的，因为 C3
  还没有拿到 BMS 的蓝牙信号；必须把板子带到工具间、接 BMS 后再验证。
- ℹ️ 当前测试固件的 WiFi 已按现场实际 SSID `CU_hSUF1` 编译；仓库模板仍保持脱敏占位符。
