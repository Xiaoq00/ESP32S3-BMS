/*
 * JK-BMS BLE client for ESP32-C3  (NimBLE / Arduino-ESP32 core 3.x)
 * 编译目标: esp32:esp32:esp32c3 (Arduino IDE 选 "ESP32C3 Dev Module")。与 S3 版逻辑完全一致, 仅设备名/编译目标不同。
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    if (!mqtt.connected()) {
      ...
      if (millis()-mt > 5000) {
        ...
        if (mqtt.connect("jk-esp32c3", nullptr, nullptr, MQTT_TOPIC, 0, true, "{\"online\":false}")) {
